...
import { Guru, KopSuratConfig, PeriodeAjaran } from '../types';
...
export const ProfilView: React.FC<ProfilViewProps> = ({
  ...
  const [localKkm, setLocalKkm] = useState<number>(kkm);
  const [kkmSuccess, setKkmSuccess] = useState(false);
  ...
  return (
    <div className="space-y-4 pb-8 max-w-4xl">
      ...
      {/* Kalender Akademik & Semester Card */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-start sm:items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-xs text-slate-900">Periode Akademik & Kalender Belajar</h3>
                <span className="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-extrabold">
                  Aktif
                </span>
              </div>
              <p className="text-xs text-slate-600 mt-0.5 font-medium">
                Tahun Ajaran <span className="font-bold text-slate-900">{periodeAktif.tahunAjaran}</span> • Semester <span className="font-bold text-slate-900">{periodeAktif.semester}</span>
              </p>
            </div>
          </div>
          ...
          {onOpenSemesterModal && (
            <button
              ...
              className="px-3.5 py-1.5 bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-bold rounded-xl shadow-2xs transition flex items-center justify-center gap-1.5 self-start sm:self-auto cursor-pointer"
            >
      ...
      <div className="bg-white rounded-xl border border-blue-100 p-4 shadow-2xs space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-xs text-slate-900">Standar Kriteria Ketuntasan Minimal (KKM / KKTP)</h3>
              ...
              <p className="text-[11px] text-slate-500 mt-0.5">
                Batas nilai minimum tuntas yang disinkronkan ke Leger Nilai, cetak rekap, dan laporan WhatsApp
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 self-start sm:self-auto">
            <input
              ...
              className="w-16 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 text-xs font-bold text-blue-700 text-center focus:ring-2 focus:ring-blue-500 font-mono"
            />
            <button
              type="button"
              onClick={() => {
                const val = Math.min(100, Math.max(0, Math.round(localKkm)));
                setLocalKkm(val);
                if (onUpdateKKM) {
                  onUpdateKKM(val, true);
                }
                setKkmSuccess(true);
                confetti({ particleCount: 40, spread: 50, origin: { y: 0.6 } });
                setTimeout(() => setKkmSuccess(false), 2500);
              }}
        ...
        <div className="flex items-center gap-1.5 pt-2 border-t border-slate-100">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Preset Cepat:</span>
